...
export class MailService {
    ...
    if (!forceNew) {
      ...
      if (stored && stored.address) {
        ...
        if (stored.provider === 'mailtm') {
          stored.apiBase = MailTmClient.getApiBase(stored.apiBase);
          StorageManager.setCurrentAccount(stored);
        }
        return stored;
      }
    }
   ...
   * Note: 1secmail is intentionally excluded — its public API was shut down.
   */
  static async getAvailableDomains(): Promise<MailDomain[]> {
    const allDomains: MailDomain[] = [];

    // Fetch from all top providers concurrently
    const [mailtmRes, inboxesRes, tempmailLolRes, guerrillaRes] = await Promise.allSettled([
   ...
   * Routing prefers the explicit provider hint (set by the custom-inbox modal)
   * and otherwise falls back to exact domain matching — never loose substrings.
   */
  static async createCustomAccount(
    ...
    const isInboxes = providerHint === 'inboxes' || (!providerHint && INBOXES_KNOWN_DOMAINS.includes(normalizedDomain));

    if (isGuerrilla) {
      const account = await GuerrillaMailClient.createAccount(cleanPrefix, normalizedDomain);
      StorageManager.setCurrentAccount(account);
      return account;
    } else if (isInboxes) {
      const account = await InboxesClient.createAccount(cleanPrefix, normalizedDomain);
      StorageManager.setCurrentAccount(account);
      return account;
    } else {
      const account = await MailTmClient.createAccount(cleanPrefix, normalizedDomain);
      StorageManager.setCurrentAccount(account);
      return account;
    }
  }
  ...
  static async getMessages(account: MailAccount, isInitialLoad: boolean = false): Promise<MailMessage[]> {
    ...
    try {
      ...
      if (err?.message === 'UNAUTHORIZED' && account.provider === 'mailtm') {
        // Token expired — silently renew it with the stored password so the
        // user keeps their address (previously this silently swapped in a
        // brand-new mailbox, losing the user's address without consent).
        if (account.password) {
          try {
            const freshToken = await MailTmClient.login(account.address, account.password, account.apiBase);
            account.token = freshToken;
            StorageManager.setCurrentAccount(account);
            remoteMessages = await MailTmClient.getMessages(freshToken, account.apiBase);
            fetchSucceeded = true;
          } catch (renewErr) {
            console.warn('Token renewal failed, using cached messages:', renewErr);
            throw err;
          }
        } else {
          throw err;
