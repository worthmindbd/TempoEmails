...
export function extractOtpCode(subject: string = '', body: string = ''): string | null {
  ...
  // 3. Subject-first check for standalone 4-8 digit numbers (skip years like 2024/2026)
  const subjectNumMatch = subject.match(/\b([0-9]{4,8})\b/);
  if (subjectNumMatch && subjectNumMatch[1] && !/^(19|20)[0-9]{2}$/.test(subjectNumMatch[1])) {
  ...
  // 5. Standalone 4, 5 or 8 digit numbers in body (skip years like 1998/2024)
 ...
 * SECURITY: only http(s) links are ever returned — this value is assigned to an anchor href.
 */
export function extractVerificationLink(html: string = '', text: string = ''): string | null {
  ...
  if (text) {
    ...
    if (textUrlMatch && textUrlMatch[1] && isSafeUrl(textUrlMatch[1])) {
